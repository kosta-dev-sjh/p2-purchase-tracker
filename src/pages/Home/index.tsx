...
const KpiStrip = React.lazy(async () => ({ default: (await import("./components/KpiStrip")).KpiStrip }));
const PlatformDonut = React.lazy(async () => ({
  default: (await import("./components/PlatformDonut")).PlatformDonut,
}));
const TrendChart = React.lazy(async () => ({ default: (await import("./components/TrendChart")).TrendChart }));
...
const ChartFallbackCard = styled.div`
  min-height: 212px;
...
export const HomePage: React.FC = () => {
  ...
  const data = useMemo(() => buildHomeData(deferredRows, month), [deferredRows, month]);
  ...
  const pickerMinYear = useMemo(
    () => computeMinYear(deferredRows.map((row) => row.date)),
    [deferredRows]
  );
  // 미래 거래(과거 데이터 정합 케이스)가 있으면 그 월까지 노출. 새 거래는 거래일자 maxDate로 차단됩니다.
  const pickerMaxMonth = useMemo(
    () => computeMaxMonthKey(deferredRows.map((row) => row.date)),
    [deferredRows]
  );
  const markedMonthKeys = useMemo(() => {
    const monthKeys = deferredRows
      .map((row) => {
  ...
  useEffect(() => {
     ...
     */
    const monthRows = deferredRows.filter((r) => {
    ...
    if (!cached || cached.hash !== hash) {
      ...
      const essentialsContext = buildInsightsRulesContext(deferredRows, month);
  ...
  }, [deferredRows, month, data.insights, getInsight, setInsight]);
