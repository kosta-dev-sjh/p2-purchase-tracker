...
export const CATEGORY_CONCEPTS: CategoryConcept[] = [
  ...
  {
    /*
     * 공과금(전기·가스·수도·난방). 통신비는 의도적으로 telecom 으로 분리 — 가계부에서
     * "공과금" 이라고 할 때 통신비는 따로 보고 싶다는 사용자 의견이 다수.
     * 관리비는 maintenance concept 으로 분리(2026-04-28) — "공과금 ≠ 관리비" 라고
     * 보는 사용자가 많아 카테고리도 둘로 떼어 합계가 명확히 나오게.
     */
    id: "utility",
    aliases: ["공과금", "전기", "가스", "수도", "utility", "bill"],
    patterns: [
      /한국전력|한전|kepco/i,
      /전기요금|전기료/i,
      /도시가스|가스요금|가스료/i,
  ...
  {
    /*
     * 관리비(아파트·오피스텔·상가). 공과금과는 별도 — 한 달 흐름에서 관리비는 단일 큰
     * 청구라 분리해야 가계부 가독성이 좋아집니다.
     *
     * 패턴 확장(2026-04-28 사용자 피드백): 한국 카드 명세서에 자주 찍히는 long-tail 보강.
     *  - "OO아파트", "OO빌라", "OO타워" 자체는 너무 광범위(거주지 외에도 가맹점 이름에
     *    "타워" 들어가는 식당 등이 많음) 라 빼고, "관리비"·"관리단" 류 결제 표기를 우선.
     *  - 카드사가 관리비 자동납부를 "OO관리비", "OO아파트관리비", "주식회사 OO관리" 등으로
     *    찍는 케이스가 다수.
     */
    id: "maintenance",
    aliases: ["관리비", "아파트관리비", "maintenance"],
    patterns: [
      /관리비|아파트관리비|오피스텔관리비|상가관리비|빌라관리비/i,
      /입주자\s*대표회의|관리사무소|관리단/i,
      /주거비|월세관리/i,
      // 자동납부 표기 — "에버리치관리(주)", "한국주택관리" 처럼 가맹점명에 "관리" 가 단독으로
      // 들어오는 케이스는 false positive 위험이 커서 "관리" 단독 매칭은 의도적으로 안 함.
      /주택\s*관리|아파트\s*관리|단지\s*관리/i,
    ],
    fallbackStandard: "living",
  },
  {
    /*
     * 교육비(학원·인강·도서·교육서비스). 자녀 교육비, 본인 자기계발, 책 구매까지 한 바구니.
     *
     * 패턴 확장(2026-04-28 사용자 피드백): 한국 가맹점명에서 자주 등장하는 키워드 long-tail.
     *  - "OO학원/OO어학원/OO영어/OO수학/OO국어" 류 — 한국 사교육 시장 가맹점 표기 관행.
     *  - "OO스쿨/OO아카데미" — 학원 브랜드가 영어로 표기되는 경우.
     *  - "독서실/스터디카페" — 학습 공간 결제, 가계부에서 교육비로 묶는 게 일반적.
     *  - 대형 교육 플랫폼/학습지: 시매쓰, 구몬, 빨간펜, 눈높이, 윤선생, 천재교육 등.
     *  - 대학교·대학원 등록금 (가맹점명에 "대학교"·"대학원" 들어가는 케이스).
     *  - 어린이집·유치원은 양육비 성격이 강하지만 가계부 분류상 교육비에 합류시키는 경우가
     *    다수라 포함. (사용자가 다르게 분류하고 싶으면 거래 상세에서 직접 변경)
     */
    id: "education",
    aliases: ["교육", "학원", "교육비", "education", "academy", "학습"],
    patterns: [
      /학원|어학원|입시|학습지/i,
      /과외|튜터링|tutor|개인교습/i,
      /메가스터디|이투스|대성학원|시대인재|이지업스쿨|종로학원|재수학원/i,
      /해커스|hackers|파고다|pagoda|YBM|와이비엠/i,
      /방송통신대|한국방송통신대|사이버대|cyber\s*univ/i,
      /coursera|udemy|인프런|inflearn|패스트캠퍼스|fastcampus/i,
      /클래스101|class101|탈잉|taling/i,
      /시매쓰|구몬|빨간펜|눈높이|윤선생|천재교육|대교/i,
      /온라인\s*강의|인강|동영상\s*강의|강의/i,
      /yes24|예스24|교보문고|영풍문고|알라딘|aladin|반디앤루니스/i,
      /도서|서점|book ?store/i,
      /독서실|스터디\s*카페|study\s*cafe/i,
      // 학교 단계 키워드. "OO대학교", "OO고등학교" 의 등록금/공납금 결제가 카드 명세서에
      // "학교 등록금" / "학교명+등록" 형태로 찍히는 케이스를 포착.
      /대학교|대학원|등록금|공납금/i,
      /어린이집|유치원|놀이학교/i,
      // 영어로 끝나는 일반 명사 매칭은 오탐(영문 가맹점명) 폭이 커 "english/math" 단독은
      // 제외하고, 한글 결합형(영어교실/수학공부방 등) 만 잡는 게 안전.
      /영어\s*교실|수학\s*공부방|논술\s*학원/i,
    ],
    fallbackStandard: "etc",
  },
  {
