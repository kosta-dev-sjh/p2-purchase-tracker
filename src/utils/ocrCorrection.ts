 ...
 *
 * ### 적용 대상: **단일 한글 글자가 3개 이상** 공백으로 끊어진 런(run)만 재결합
 *
 * 대상 패턴 예: `배 송 완 료`, `취 소 완 료`, `반 품 신 청`, `무 료 선 물 포 장`
 * 대상 아닌 예: `루디크 루브르`, `로켓 내일`, `체리 블라썸` — 각 토큰이 2글자 이상이라
 *              Tesseract 분할 아티팩트가 아니고, 실제 UI에서 띄어쓰기로 분리된 **정상
 *              다단어**입니다. 여기까지 합쳐 버리면 `"루디크루브르감성여행미니기내용캐리어"`
 *              처럼 상품명 가독성이 붕괴되고 사용자가 fixture를 기준으로 기대하던 prefix 와
 *              어긋납니다.
 *
 * 정규식 설계:
 *   - `(?<![가-힣])` / `(?![가-힣])` : 런 양끝이 한글 경계 밖(공백·영문·숫자·줄 시작/끝)
 *   - `[가-힣](?:[\t ]+[가-힣]){2,}` : 단일 한글 + `공백 + 단일 한글` 을 최소 2회 반복
 *     → 최소 3개의 단일 한글이 공백으로 끊어진 런만 잡힙니다.
