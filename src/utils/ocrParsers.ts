...
export function parseCoupangOrderText(rawText: string): PurchaseOCRResult[] {
  ...
  const COUPANG_BARE_BADGES = /로켓|판매자|새벽|내일|대일|오늘|당일|도착|배송|와우|광고|쿠폰|추천|적립|할인|BEST|NEW|HOT|SALE/;
  ...
  const COUPANG_GARBAGE_HEAD_PATTERNS = [
    ...
    /[A-Za-z0-9]{1,4}(?=\s+(?:로켓|판매자|새벽|내일|대일|오늘|당일|와우|무료|해외))\s+/,
  ...
  const COUPANG_BADGE_ATOMS = /로켓|판매자|새벽|내일|대일|오늘|당일|와우|무료|해외|쿠팡|쿠폰|도착|배송|광고|추천|적립|할인|프레시|프레쉬|그로스|직구|설치|플러스|멤버십|캐시|카드/g;
