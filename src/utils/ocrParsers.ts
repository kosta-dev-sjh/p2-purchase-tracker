...
export function parseCoupangOrderText(rawText: string): PurchaseOCRResult[] {
  ...
  //
  // 2026-04-23: "로켓배송 내일도착 상품이름" → "도착 상품이름" 으로 꼬리 한 조각이 잔존하는
  // 회귀가 관찰돼 **복합 태그와 꼬리 단어**를 추가했습니다. 아래 원칙대로 배열했습니다.
  //   (i)  복합 태그(여러 단어가 공백 없이/있이 붙는 형태)를 **원자 태그보다 먼저** 둬
  //        "로켓\s*배송" 이 "로켓" 에 선점되지 않도록 합니다.
  //   (ii) "도착", "배송", "당일" 같은 꼬리 단어를 원자로 추가합니다. 이 단어들은
  //        실제 쿠팡 상품명 선두에 등장하는 일이 거의 없어 false-positive 위험이 낮습니다.
  //   (iii) "와우(?: 멤버십)?" 를 추가합니다(로켓 와우 카드 뱃지가 남는 케이스).
  //   (iv) 리딩 기호 char class 에 `~`, `@`, `#`, `&`를 추가합니다. 쿠팡 스크린샷에서
  //        🚀 가 드물게 이 기호들로 치환되는 케이스가 관찰됐습니다.
  const leadingTagRegex =
    /^(?:[🚀↑↓▲▼★☆·•»«‹›<>;:,."'”“‘’\-\|ㅣ=_*©!?~@#&]+\s*|[a-zA-Z]{1,2}\s*[>»‹<]+\s*|판매자\s*로켓|로켓\s*(?:직구|프레시|배송|설치|와우|프레쉬)|내일\s*(?:도착|배송)|오늘\s*(?:도착|배송)|새벽\s*(?:도착|배송)|당일\s*(?:도착|배송)|무료\s*배송|와우\s*멤버십|로켓|판매자|새벽|내일|대일|오늘|당일|도착|배송|와우)\s*/;
